...
public class Ratings {
	 ...
	 * 
	 * See http://recsyswiki.com/wiki/Root_mean_square_error and
	 * http://recsyswiki.com/wiki/Mean_absolute_error
	 * 
	 */
	 ...
	 * 
	 * For CBD (capped binomial deviance), see
	 * http://www.kaggle.com/c/ChessRatings2/Details/Evaluation
	 * 
	 * If the recommender can take time into account, and the rating dataset
	 * provides rating times, this information will be used for making rating
	 * predictions.
	 * 
	 * Literature: Ken Goldberg, Theresa Roeder, Dhruv Gupta, and Chris Perkins:
	...
	public static RatingPredictionEvaluationResults evaluate(
		...
		if (recommender instanceof ITimeAwareRatingPredictor
				&& ratings instanceof ITimedRatings) {
			...
			for (int index = 0; index < ratings.size(); index++) {
				double prediction = time_aware_recommender.predict(
						timed_ratings.users().get(index), timed_ratings.items()
								.get(index), timed_ratings.times().get(index));
				double error = prediction - ratings.get(index);

				// System.out.printf("%d - %d - %f",
				// timed_ratings.users().get(index),
				// timed_ratings.items().get(index), prediction);
				// System.out.println();
			...
			for (int index = 0; index < ratings.size(); index++) {
				double prediction = recommender.predict(
						ratings.users().get(index), ratings.items().get(index));
				double error = prediction - ratings.get(index);
				rmse += error * error;
				mae += Math.abs(error);
				cbd += computeCBD(ratings.get(index), prediction,
		...
		result.put("MAE", mae);
		result.put("NMAE",
				mae / (recommender.getMaxRating() - recommender.getMinRating()));
		result.put("CBD", cbd);
		return result;
	}

	// HACK to extract pref information
	public static RatingPredictionEvaluationResults evaluate(
			IRatingPredictor recommender, IRatings ratings,
			PrefDataBaseInList<PrefDataBaseIn> listPrefData, int fold) {
		double rmse = 0;
		double mae = 0;
		double cbd = 0;
		...
		if (recommender instanceof ITimeAwareRatingPredictor
			...
			for (int index = 0; index < ratings.size(); index++) {
				...
				// TODO cricia user, item, prediction
				// System.out.printf("%d;%d;%f", ratings.users().get(index),
				// ratings.items().get(index), prediction);
				// System.out.println();
	...
	public static double computeCBD(double actual_rating, double prediction,
		...
		return -(actual_rating * Math.log10(prediction) + (1 - actual_rating)
				* Math.log10(1 - prediction));
	}
